...
import { Search, Bell, Settings, User, Clock, Star, ChevronLeft } from 'lucide-react';
...
const TopBar: React.FC<TopBarProps> = ({
  ...
  // React Router v6 stores an index in history.state — idx 0 means the user
  // landed directly on this page (no prior in-app history to go back to).
  ...
  return (
    <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8 shadow-sm flex-shrink-0">
      {/* Left: Back Button + Page Title Area */}
      <div className="flex items-center gap-3 h-full">
        {!isHome && (
          <button
            onClick={handleBack}
            className="p-1.5 -ml-1.5 rounded-md text-gray-400 hover:text-brand-orange hover:bg-orange-50 transition-colors"
            aria-label="Go back"
          >
            <ChevronLeft size={20} />
          </button>
